 ...
 *
 * Centralized configuration for CLI tools, scaffolding, validation, and build processes.
 * This file contains settings used by build scripts, generators, and validation tools.
 *
 * Type definitions are centralized in the unified type system
 * at src/lib/types/config.ts and imported via the $types alias.
 *
 * @module settings-scripts
 */

import type { ScriptsSettings } from "$types";

/**
 * Scripts Settings - Build/Generation Configuration
 *
 * Contains all configuration for:
 * - Validation (TypeScript, ESLint, Mermaid)
 * - Content generation (menu, search index, flat navigation)
 * - Scaffolding (content templates, minimums)
 * - Schema generation (JSON Schema from Zod)
 * - Common utilities (file extensions, config paths)
 */
export const SCRIPTS_SETTINGS: ScriptsSettings = {
	validation: {
		generated: {
