...
describe("FlatNavGenerator", () => {
	...
	describe("Flat Navigation Generation", () => {
		it("should generate correct number of navigation entries", async () => {
			...
			expect(outputContent).toContain("totalCount: 9"); // 8 chapters + 1 book overview
		}, 10000);
	...
	describe("TypeScript Code Generation", () => {
		...
		it("should include generation metadata and home URL", async () => {
			...
			expect(outputContent).toContain(
				`Book Overview: ${SETTINGS.scripts.flatNav.bookOverview.chapterUrl}`
			);
		}, 10000);
...
export const contentMenu: MenuStructure = {
	...
	units: [
		{
			...
			const outputContent = readFileSync(testSetup.outputPath, "utf-8");
			expect(outputContent).toContain("totalCount: 1"); // 0 chapters + 1 book overview
		}, 10000);
		...
		it("should handle minimal content structure", async () => {
			...
			const outputContent = readFileSync(testSetup.outputPath, "utf-8");
			expect(outputContent).toContain("totalCount: 3"); // 2 chapters + 1 book overview
		}, 10000);
	...
	describe("Integration with Configuration", () => {
		it("should respect configuration settings", async () => {
			...
			// Verify book overview configuration matches settings
			expect(flatNavSettings.bookOverview.id).toBe(SETTINGS.scripts.flatNav.bookOverview.id);
			expect(flatNavSettings.bookOverview.chapterUrl).toBe(
				SETTINGS.scripts.flatNav.bookOverview.chapterUrl
			);
			expect(flatNavSettings.bookOverview.title).toBe(SETTINGS.scripts.flatNav.bookOverview.title);
			expect(flatNavSettings.bookOverview.filePath).toBe(
				SETTINGS.scripts.flatNav.bookOverview.filePath
			);
		}, 10000);
	});
