...
class StateManager:
    ...
    def get_prompt_execution_summary(self, prompt_id: str, agent_name: str) -> Dict[str, Any]:
        ...
        if not prompt_states:
            return {
                ...
                'last_execution_status': 'never_run',
                'total_executions': 0,
                'completed_executions': 0,
                'failed_executions': 0,
                'completed_units': 0,
                'failed_units': 0,
                'has_resumption_options': False,
        ...
        summary = {
            ...
            'completed_units': len([s for s in prompt_states if s.status == 'completed' and s.unit]),
            'failed_units': len([s for s in prompt_states if s.status == 'failed' and s.unit]),
            'has_resumption_options': len(prompt_states) > 0,
            'units': {}
        }
        
        # Get last execution time and status
        last_state = max(prompt_states, key=lambda s: s.end_time or s.start_time or '1970-01-01T00:00:00')
        summary['last_execution_status'] = last_state.status
        summary['last_execution_time'] = last_state.end_time or last_state.start_time
        ...
        if summary['completed_executions'] == 0:
            if summary['failed_executions'] == 0:
                summary['overall_status'] = 'never_run'
            else:
    ...
    def clear_prompt_execution_state(self, prompt_id: str, agent_name: str, units_to_clear: Optional[List[str]] = None) -> bool:
        ...
        if units_to_clear is None:
            ...
            logger.info(f"Clearing all execution state for prompt {prompt_id}")
        else:
            # Clear only specified units
            filtered_states = []
            for state in states:
                if state.prompt_id == prompt_id and state.unit in units_to_clear:
                    logger.info(f"Clearing execution state for prompt {prompt_id}, unit {state.unit}")
                    continue  # Skip (remove) this state
                filtered_states.append(state)
        
        success = self.save_execution_states(agent_name, filtered_states)
        cleared_count = initial_count - len(filtered_states)
        
        if success:
            logger.info(f"Successfully cleared {cleared_count} execution states for prompt {prompt_id}")
        else:
            logger.error(f"Failed to clear execution states for prompt {prompt_id}")
