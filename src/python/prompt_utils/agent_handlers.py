...
class ClaudeHandler(BaseAgentHandler):
    ...
    def build_command(self, prompt: str, unit: Optional[str] = None, 
        ...
        if output_format:
            ...
            # Enable streaming features for JSON output
            if output_format == 'stream-json':
                # For stream-json, always add verbose and partial messages
                cmd.append("--verbose")
                cmd.append("--include-partial-messages")
                self.logger.info("Enabled streaming JSON with verbose and partial messages")
            elif output_format == 'json':
                # Regular JSON output - no additional flags needed
                # --include-partial-messages is only valid for stream-json
                self.logger.info("Enabled standard JSON output")
