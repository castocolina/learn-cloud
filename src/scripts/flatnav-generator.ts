...
// Use destructuring for cleaner settings access
...
export class FlatNavGenerator {
	...
	constructor(inputPath?: string, outputPath?: string) {
		this.inputPath = inputPath || flatNavSettings.paths.inputFile;
		this.outputPath = outputPath || flatNavSettings.paths.outputFile;
	}
	...
	private generateFlatEntries(): void {
		...
		for (const unit of this.contentMenu.units) {
			if (flatNavSettings.navigation.skipEmptyUnits && unit.chapters.length === 0) {
 ...
 * Home URL: ${flatNavSettings.navigation.homeUrl}
 *
...
export const homeUrl = "${flatNavSettings.navigation.homeUrl}";
`;
	}
	...
	private async runValidation(): Promise<void> {
		...
		try {
			await runGeneratedFileValidation(this.outputPath);
		} catch (error) {
