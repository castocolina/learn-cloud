...
- **RECURRING ISSUES DOCUMENTATION**: When generating prompts for debugging/error resolution tasks that represent RECURRING patterns (issues that have appeared 2+ times), ALWAYS include instructions for the agent to document findings in `RECURRING-ISSUES.md`. One-time bugs should NOT be documented to maintain focus on architectural requirements.
